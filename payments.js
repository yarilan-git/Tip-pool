...
// Add a payment delete button
function appendDelBtn(tr) {

  const removePaymentBtn = document.createElement("button");
  removePaymentBtn.innerText = "X";
  removePaymentBtn.addEventListener("click", removePayment);
  const removePaymentTd = document.createElement("td");
  removePaymentTd.appendChild(removePaymentBtn);
  tr.appendChild(removePaymentTd);
}
...
const removePayment = function(e) { 
  
    let payId = (e.target.parentElement).parentElement.getAttribute('id');  
    delete allPayments[payId];  
